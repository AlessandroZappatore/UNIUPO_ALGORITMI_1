...
{
    const entry_t *ptr1=a;
    const entry_t *ptr2=b;
...
int by_year_comparator(const void *a, const void *b)
{
    ...
    else if(ptr1->year==ptr2->year) return 0;
    else return -1;
}
...
int by_track_number_comparator(const void *a, const void *b)
{
    const entry_t *ptr1=a;
    const entry_t *ptr2=b;
    if(ptr1->track_num>ptr2->track_num) return 1;
    else if(ptr1->track_num==ptr2->track_num) return 0;
    else return -1;
}
...
int by_track_title_comparator(const void *a, const void *b)
{
    const entry_t *ptr1=a;
    const entry_t *ptr2=b;
    return strcmp(ptr1->track_title, ptr2->track_title);
}
...
void playlist_sort(playlist_t playlist, playlist_sorting_criterion_t order_by)
{
    ...
    switch (order_by)
    {
        case playlist_by_artist_sorting_criterion:
            //upo_quick_sort(playlist->entries, playlist->size, entry_size, by_artist_comparator);
            upo_merge_sort(playlist->entries, playlist->size, entry_size, by_artist_comparator);
            //upo_insertion_sort(playlist->entries, playlist->size, entry_size, by_artist_comparator);
            break;
        case playlist_by_album_sorting_criterion:
            //upo_quick_sort(playlist->entries, playlist->size, entry_size, by_album_comparator);
            upo_merge_sort(playlist->entries, playlist->size, entry_size, by_album_comparator);
            //upo_insertion_sort(playlist->entries, playlist->size, entry_size, by_album_comparator);
            break;
        case playlist_by_year_sorting_criterion:
            //upo_quick_sort(playlist->entries, playlist->size, entry_size, by_year_comparator);
            upo_merge_sort(playlist->entries, playlist->size, entry_size, by_year_comparator);
            //upo_insertion_sort(playlist->entries, playlist->size, entry_size, by_year_comparator);
            break;
        case playlist_by_track_number_sorting_criterion:
            //upo_quick_sort(playlist->entries, playlist->size, entry_size, by_track_number_comparator);
            upo_merge_sort(playlist->entries, playlist->size, entry_size, by_track_number_comparator);
            //upo_insertion_sort(playlist->entries, playlist->size, entry_size, by_track_number_comparator);
            break;
        case playlist_by_track_title_sorting_criterion:
            //upo_quick_sort(playlist->entries, playlist->size, entry_size, by_track_title_comparator);
            upo_merge_sort(playlist->entries, playlist->size, entry_size, by_track_title_comparator);
            //upo_insertion_sort(playlist->entries, playlist->size, entry_size, by_track_title_comparator);
            break;
        default:
